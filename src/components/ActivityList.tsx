...
export default function ActivityList({ activities, dispatch }: ActivityListProps) {
  ...
  return (
    <>
      ...
      <ul>
        {isEmptyActivities ?
          <p className="text-center text-2xl my-2 mt-10">No hay actividades aún...</p> :
          ...
          >
            ...
            <div className="flex gap-5 items-center">
              <button
              onClick={() => dispatch({ type: "set-activeID", paylod: { id: activity.id } })}
              >
                <PencilSquareIcon className="w-8 h-8 text-gray-800" />
              </button>

              <button
              onClick={() => dispatch({ type: "delete-activity", paylod: { id: activity.id } })}
              >
